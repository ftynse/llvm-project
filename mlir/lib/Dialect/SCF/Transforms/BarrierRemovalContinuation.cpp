...
static void splitBlocksWithBarrier(Region &region) {
  auto barriers = llvm::to_vector<4>(region.getOps<scf::BarrierOp>());
  for (scf::BarrierOp op : barriers) {
...
static void traverseUntilBarrier(Block *start,
  ...
  while (!stack.empty()) {
    ...
    if (isa_and_nonnull<scf::BarrierOp>(
  ...
  for (Block *block : blocks) {
    ...
    for (Operation &op : *block) {
      ...
      if (auto branch = dyn_cast<BranchOp>(&op)) {
        ...
        if (isa_and_nonnull<scf::BarrierOp>(branch->getPrevNode())) {
...
static void reg2mem(ArrayRef<llvm::SetVector<Block *>> subgraphs,
  ...
  for (Value value : valuesToStore) {
    Value allocation =
        allocateTemporaryBuffer(allocaBuilder, value, iterationCounts);
...
static void createContinuations(scf::ParallelOp parallel, Value storage) {
  ...
  for (Block &block : outerExecuteRegion.region()) {
    if (!isa_and_nonnull<scf::BarrierOp>(block.getTerminator()->getPrevNode()))
      continue;
...
std::unique_ptr<Pass> mlir::createBarrierRemovalContinuation() {
