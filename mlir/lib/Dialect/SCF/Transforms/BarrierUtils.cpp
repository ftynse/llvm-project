...
void findValuesUsedBelow(scf::BarrierOp barrier,
                         llvm::SetVector<Value> &crossing) {
...
void findValuesUsedBelow2(scf::BarrierOp barrier, DominanceInfo &dominanceInfo,
...
Value allocateTemporaryBuffer(OpBuilder &builder, Value value,
  ...
  Value alloc =
      builder.create<memref::AllocaOp>(value.getLoc(), type, iterationCounts);
  return alloc;
}
