...
void AffineParallelOp::build(OpBuilder &builder, OperationState &result,
  ...
  // Concatenates maps defined in the same input same (same dimensions and
...
static void deduplicateAndResolveOperands(
  ...
  for (const auto &list : operands) {
    ...
    for (Value operand : valueOperands) {
      auto it = llvm::find(uniqueOperands, operand);
      unsigned pos;
      if (it != uniqueOperands.end()) {
        pos = std::distance(uniqueOperands.begin(), it);
      } else {
        pos = uniqueOperands.size();
