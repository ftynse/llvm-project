...
/// Fixed-size buffer for a 64-byte AMDGPU kernel descriptor. A `std::array`
/// (rather than a `MutableArrayRef<uint8_t>`) lets the compiler enforce the
/// size at the call site instead of a runtime check.
...
// the .rodata KD blob is unreachable. We log the underlying Error here and
// leave `Meta.HasKernelDescriptor == false` -- the caller is contractually
...
int64_t nodeAsInt(const llvm::msgpack::DocNode &N) {
...
std::optional<uint32_t> nodeAsUInt32(const llvm::msgpack::DocNode &N) {
  if (N.getKind() == llvm::msgpack::Type::Int) {
    int64_t V = N.getInt();
    if (V < 0 || V > UINT32_MAX)
      return std::nullopt;
...
llvm::Expected<KernelMeta> extractKernelMeta(llvm::MemoryBufferRef ElfData,
  ...
  // (raiser) is responsible for refusing the lift if the field is false
