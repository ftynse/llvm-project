...
func.func @conv(
    ...
    %bias_init: !toutput {bufferization.writable = true},
...
//   %0 = transform.structured.match ops{["linalg.conv_2d_nhwc_fhwc"]} in %arg0 : (!transform.any_op) -> !transform.any_op
...
transform.sequence failures(propagate) {
^bb0(%arg0: !transform.any_op):
  %conv = transform.structured.match ops{["linalg.conv_2d_nhwc_fhwc"]} in %arg0 : (!transform.any_op) -> !transform.any_op
  %1 = transform.structured.match ops{["linalg.generic"]} in %arg0 : (!transform.any_op) -> !transform.any_op
  %bias, %relu = transform.split_handle %1 : (!transform.any_op) -> (!transform.any_op, !transform.any_op)

  // transform.test_print_remark_at_operand %conv, "convolution" : !transform.any_op
  // transform.test_print_remark_at_operand %bias, "bias" : !transform.any_op
  // transform.test_print_remark_at_operand %relu, "relu" : !transform.any_op
  ...
  %co_loop, %relu2 = transform.structured.tile_to_forall_op %relu tile_sizes [0, 0, 0, 64] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
  ...
  %multi_loop, %relu3 = transform.structured.tile_to_forall_op %relu2 tile_sizes [1, 0, 1, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
  // this would have given us co, xo, no, yo, [ni=1, xi, yi=1, ci] if we tiled X before
  // so we tile it after

  %relu4, %xo_loop = transform.structured.tile_to_scf_for %relu3 [0, 5, 0, 0] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
  ...
  %conv2, %co_loop2 = transform.structured.fuse_into_containing_op %conv into %co_loop : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
  ...
  transform.structured.fuse_into_containing_op %bias into %co_loop2 : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
  ...
  // transform.structured.decompose %conv3
  // TODO: NEW putting only `!transform.any_op` here leads to a crash
  transform.structured.generalize %conv3 : (!transform.any_op) -> !transform.any_op

  // TODO: loop unroll must invalidate handles properly...
  // transform.loop.unroll %loops#6 { factor = 8 } : !transform.any_op
  // transform.loop.unroll %loops#5 { factor = 2 } : !transform.any_op
  // %convs = transform.structured.match ops{["linalg.conv_2d_nhwc_fhwc"]} in %arg0 : (!transform.any_op) -> !transform.any_op
  // transform.structured.decompose %convs
